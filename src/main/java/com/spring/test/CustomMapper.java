...
public interface CustomMapper {
	...
	public CustomAcc selectBNCustom(String busiNum);

	// custom 이용해 조회
	@Select("select * from custom cu join account a on cu.busi_num = a.busi_num  where custom = #{custom}")
	public CustomAcc selectOneCustom(String custom);
	...
	// 거래처 수정하기
//	@Update("update custom set custom = #{custom} where busi_num = #{busiNum}")
	@Update("update custom set custom = #{custom}, short = #{sHort}, ceo = #{ceo}, charge_person = #{chargePerson}, "
...
//	public int updateCustom(String busiNum, String custom);

	// 거래처 계좌정보 수정하기
	@Update("update account set factory = #{factory}, trade_bank = #{tradeBank}, account_num = #{accountNum} where busi_num = #{busiNum}")
